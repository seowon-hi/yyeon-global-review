...
import { COLOR_RECOMMENDATION_LABELS } from "../i18n/colorRecommendation";
...
export function ColorRecommendation({ currentLang }: ColorRecommendationProps) {
  ...
  useEffect(() => {
    const fetchColorGuide = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch("/data/color_guide.json");
        if (!response.ok) {
  ...
  const t =
    COLOR_RECOMMENDATION_LABELS[currentLang] || COLOR_RECOMMENDATION_LABELS.EN;
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-rose-50 border border-rose-100 rounded-2xl text-center space-y-2">
        ...
        <p className="text-[10px] text-rose-400 max-w-sm font-medium">
          {error}
        </p>
      </div>
    );
  ...
  const filteredColors = data.colors.filter((c) =>
                ...
                <div className="col-span-8 p-4 flex flex-col justify-between space-y-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-gray-500 font-medium leading-relaxed">
                      {colorItem.description[currentLang] ||
                        colorItem.description["KO"]}
                    </p>
                  </div>
